...
import socket
import os, sys
sys.path.append(os.path.join('Users', 'wchill', 'Projects', 'Left4VR', 'myo-python'))

myo.init()
sock = socket.create_connection(('192.168.1.156', 8000))
sock.setblocking(0)

SHOW_OUTPUT_CHANCE = 0.00
MELEEMINSPEED = 150
MELEETRUE = False
last_gyro = None
r"""
...
class Listener(myo.DeviceListener):
    ...
    def on_gyroscope_data(self, myo, timestamp, gyroscope):
        global last_gyro
        if last_gyro is None or last_gyro[2] - gyroscope[2] > 50 or last_gyro[2] - gyroscope[2] < -50:
            last_gyro = gyroscope
            print gyroscope
#        show_output('gyroscope', gyroscope)
        if gyroscope[2]  > MELEEMINSPEED or gyroscope[2] < -MELEEMINSPEED:
            melee()
        else:
            meleeReset()
